...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \If{$samples_{beg}^k[curr_{run}]=samples_{end}^k[curr_{run}]$}
    ...
    \EndIf
    \ElsIf{$(curr_{run}\neq 0 \land \neg single_{run} \land
    curr_{run}=curr_{thr})\lor curr_{run}=|samples_{beg}^k|-1$}
...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \EndFunction
  \end{algorithmic}
  \caption{Algorithm to extend a match at $col$ using $\varphi$ and
  $\varphi^{-1}$ and MS}  
\end{algorithm}
\end{document}
