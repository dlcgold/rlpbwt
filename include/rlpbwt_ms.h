...
class rlpbwt_ms {
   ...
   private:
     ...
     */
    column_ms build_column(std::string &column, std::vector<unsigned int> &pref,
                           sdsl::int_vector<> &div) {
        ...
        // variable to compute curr lcs in order to eventually compute
        // thresholds unsigned int lcs = 0; bool to check first symbol fo the
        // column
        bool start = true;
        ...
        for (unsigned int i = 0; i < height_tmp; i++) {
            ...
            if ((i == height_tmp - 1) ||
                ...
                // update bitvectors for "u" and "v" and swap the case to study
                // in next run
                if (pusho) {
                    if (v != 0) {
                        onevec[v - 1] = true;
        ...
        return {start,  count0, runvec,     zerovec,
                onevec, thr,    sample_beg, sample_end};
    }
    ...
    unsigned int lf(unsigned int col_index, unsigned int index, char symbol,
        ...
        if (verbose) {
            ...
            std::cout << uv.first << " " << uv.second
                      << " with c: " << this->cols[col_index].count_0 << "\n";
        }
    ...
    std::pair<unsigned int, unsigned int> uvtrick(unsigned int col_index,
        ...
        // two main cases:
        // - run > 1
        // - run = 0 or run = 1 (we separate this two cases due to some errors
        // using
        //   select)
        if (run > 1) {
            // if the run is an even index run it means that before we have the
            // same amount of zeros runs and ones runs
            if (run % 2 == 0) {
                ...
                // we compute the number of zeros and ones before the current
                // run using select over the two bitvectors of zeros and ones
                ...
                // depending on the first symbol of the column the offset
                // increase the number of zeros or the number of ones (they
                // increase alternating)
            ...
            if (run == 0) {
                // if we are in the first run (of index 0) simply v is always 0
                // if we are in a zero beginning column and vice-versa for u if
                // we are in a one beginning column. The non-zero value is the
                // same of index because it's the first run
                if (this->cols[col_index].zero_first) {
                    return {index, 0};
                } else {
                    return {0, index};
                ...
                // if we are in the second run (of index 1) we fix one of the
                // two values (based practically on the length of the first
                // run), according to the first symbol of the column, and we
   ...
   public:
        ...
        if (!vcf) {
            ...
            if (input_matrix.is_open()) {
                ...
                while (getline(input_matrix, line) && !line.empty()) {
                    ...
                    this->cols[count] = col;
                    // create rank/select outside build_column due to references
                    // problems
                    this->cols[count].rank_runs =
                        sdsl::sd_vector<>::rank_1_type(&this->cols[count].runs);
                    this->cols[count].select_runs =
                        sdsl::sd_vector<>::select_1_type(
                            &this->cols[count].runs);
                    this->cols[count].rank_u =
                        sdsl::sd_vector<>::rank_1_type(&this->cols[count].u);
                    this->cols[count].select_u =
                        sdsl::sd_vector<>::select_1_type(&this->cols[count].u);
                    this->cols[count].rank_v =
                        sdsl::sd_vector<>::rank_1_type(&this->cols[count].v);
                    this->cols[count].select_v =
                        sdsl::sd_vector<>::select_1_type(&this->cols[count].v);
                    this->cols[count].rank_thr =
                        sdsl::sd_vector<>::rank_1_type(&this->cols[count].thr);
                    this->cols[count].select_thr =
                        sdsl::sd_vector<>::select_1_type(
                            &this->cols[count].thr);
                ...
                this->cols[count].rank_runs =
                    sdsl::sd_vector<>::rank_1_type(&this->cols[count].runs);
                this->cols[count].select_runs =
                    sdsl::sd_vector<>::select_1_type(&this->cols[count].runs);
                this->cols[count].rank_u =
                    sdsl::sd_vector<>::rank_1_type(&this->cols[count].u);
                this->cols[count].select_u =
                    sdsl::sd_vector<>::select_1_type(&this->cols[count].u);
                ...
                this->cols[count].rank_thr =
                    sdsl::sd_vector<>::rank_1_type(&this->cols[count].thr);
                this->cols[count].select_thr =
                    sdsl::sd_vector<>::select_1_type(&this->cols[count].thr);
                    ...
                    if (curr_run == this->cols[i].sample_beg.size() - 1) {
                        ...
                        if (lce.first == next_pos) {
                            ...
                            if (i != query.size() - 1) {
                                curr_index = lf(i, curr_index, query[i]);
                                curr_run =
                                    this->cols[i + 1].rank_runs(curr_index);
    ...
    ms_matches match_thr(const std::string &query, bool extend_matches = false,
        ...
        for (unsigned int i = 0; i < query.size(); i++) {
            ...
            if (query[i] == symbol) {
                ...
                if (curr_run != 0 && this->cols[i].thr[curr_index] &&
    ...
    match_tsv_lce(const char *filename, const char *out,
        ...
        if (!vcf) {
            ...
            if (input_matrix.is_open()) {
                ...
                if (out_match.is_open()) {
                    for (unsigned int i = 0; i < queries_panel.size(); i++) {
                        ...
                        ms_matches matches;
                        matches =
                            this->match_lce(query, extend_matches, verbose);
                        if (verbose) {
                            std::cout << i << ": ";
                        }
                        out_match << i << ": ";
            ...
            bcf_hdr_t *hdr = bcf_hdr_read(fp);
            bcf1_t *rec = bcf_init();
            this->height = bcf_hdr_nsamples(hdr);
            std::vector<std::string> queries_panel;
            std::string new_column;
            while (bcf_read(fp, hdr, rec) >= 0) {
                new_column = "";
    ...
    /**
     * @brief function to compute queries with lce from a transposed tsv or vcf
     * file and output them on a file
     * @param filename queries file
     * @param out output file
     * @param extend_matches bool to extende mathc with rows values
...
#pragma omp parallel for default(none) \
    shared(queries, matches_vec, n_queries, extend_matches, verbose)
                for (unsigned int i = 0; i < n_queries; i++) {
                    matches_vec[i] =
                        this->match_lce(queries[i], extend_matches, verbose);
                }
